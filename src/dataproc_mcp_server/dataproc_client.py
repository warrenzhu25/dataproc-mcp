...
class DataprocClient:
    ...
    async def list_clusters(self, project_id: str, region: str) -> Dict[str, Any]:
        ...
        try:
            ...
            request = types.ListClustersRequest(
            ...
            response = await loop.run_in_executor(
                None, client.list_clusters, request
            )
            ...
            for cluster in response:
                clusters.append({
                    ...
                    "machine_type": cluster.config.master_config.machine_type_uri.split("/")[-1],
                    "creation_time": cluster.status.state_start_time.isoformat() if cluster.status.state_start_time else None,
    ...
    async def create_cluster(
        ...
        try:
            ...
            operation = await loop.run_in_executor(
                None, client.create_cluster, request
            )
    ...
    async def delete_cluster(
        ...
        try:
            ...
            operation = await loop.run_in_executor(
                None, client.delete_cluster, request
            )
    ...
    async def get_cluster(
        ...
        try:
            ...
            cluster = await loop.run_in_executor(
                None, client.get_cluster, request
            )
            ...
            return {
                ...
                "zone": cluster.config.gce_cluster_config.zone_uri.split("/")[-1] if cluster.config.gce_cluster_config.zone_uri else None,
                "metrics": {
                    "hdfs_capacity_mb": cluster.metrics.hdfs_metrics.capacity_mb if cluster.metrics and cluster.metrics.hdfs_metrics else None,
                    "yarn_allocated_memory_mb": cluster.metrics.yarn_metrics.allocated_memory_mb if cluster.metrics and cluster.metrics.yarn_metrics else None
                }
            }
    ...
    async def submit_job(
        ...
        try:
            ...
            job_result = await loop.run_in_executor(
                None, client.submit_job, request
            )
        ...
        try:
            ...
            for job in response:
                ...
                jobs.append({
                    ...
                    "submission_time": job.status.state_start_time.isoformat() if job.status.state_start_time else None,
    ...
    async def get_job(
        ...
        try:
            ...
            return {
                ...
                "end_time": job.status.state_start_time.isoformat() if job.status.state_start_time else None,
                "driver_output_uri": job.driver_output_resource_uri,
    ...
    async def cancel_job(
        ...
        try:
            ...
            job = await loop.run_in_executor(
                None, client.cancel_job, request
            )
