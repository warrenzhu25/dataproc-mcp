...
def get_default_project() -> Optional[str]:
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            ...
            logger.debug("Retrieved default project from gcloud config", project_id=project_id)
            return project_id
        else:
        ...
        logger.debug("gcloud CLI not found - install Google Cloud SDK to use default project")
        return None
...
def get_default_region() -> Optional[str]:
    ...
    try:
        ...
        logger.debug("gcloud CLI not found - install Google Cloud SDK to use default region")
        return None
    except Exception as e:
...
def get_default_zone() -> Optional[str]:
    ...
    try:
        ...
        logger.debug("gcloud CLI not found - install Google Cloud SDK to use default zone")
        return None
    except Exception as e:
